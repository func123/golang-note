...
​		 将栈顶向下移动4*8+7并且把参数放到SP的偏移处。其中`ANDQ	$~15, SP`，将SP和15相与，是为了让SP按16字节对其，这样做了因为CPU有一组SSE指令，这些指令使用的内存地址必须是16的倍数。
...
```go
	...
	CALL	runtime·osinit(SB)	// 只要获取CPU的核数并放在全局变量ncpu中
...
​		主要是对m0进行初始化并且初始化`procs`个p，放进全局变量`allp` 中，且将m0和`allp[0]`绑定
...
​		在m内部，通过 `mp.alllink = allm`实现一个链表，指向上一个m
...
​		函数省略了跟调度不太相关的代码。这个函数主要是初始化`nprocs`个p，用全局变量`allp`来维护一个p的数组，并且用allp[0]与m0进行绑定。至此，系统线程通过段寄存器绑定m0，其中又关联着g0，而m0又与p进行了绑定，因此g0 、m0、p 以及  thread 都关联在一起； m 和 p 中都维护着一个链表，指向上一个m和p，全局变量`schedt`中，也用变量`pidle` 和` npidle `维护着空闲的下一个p以及数量。
